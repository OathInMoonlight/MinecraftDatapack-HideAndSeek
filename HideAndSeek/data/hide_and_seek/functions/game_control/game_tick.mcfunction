...
execute as @a[team=seeker] unless data entity @s Inventory[{Slot:0b}].id run clear @s minecraft:iron_sword
execute as @a[team=seeker] unless data entity @s Inventory[{Slot:0b}].id run item replace entity @a[team=seeker] hotbar.0 with minecraft:iron_sword{Unbreakable:true,HideFlags:2}
...
execute as @a[team=hider] if score #game_start game_control matches 1 run effect give @s invisibility infinite
...
execute as @a[team=hider,scores={death_counter=1}] if score #game_start game_control matches 2 run function hide_and_seek:game_control/hider_death
