...
execute as @a[team=hider] at @s if score @s hider_points = #hider_reward_point game_control run function hide_and_seek:skill/hider_potion_reward with storage hide_and_seek:game_control initial_value
...
execute if score #game_start game_control matches 2 run execute if score #seek_time_counter_seconds game_control matches 0 run function hide_and_seek:skill/rand_pos with storage hide_and_seek:game_control tmp
